...
echo "[STEP] 1/6 health check: ${API_V1}/health"
...
echo "[STEP] 2/6 login: ${API_V1}/auth/login"
LOGIN_JSON="$(curl -sS -X POST "${API_V1}/auth/login" \
...
echo "[STEP] 3/6 create async ingest job: ${API_V1}/documents"
...
echo "[STEP] 4/6 poll ingest job until completed"
START_TS="$(date +%s)"
JOB_JSON=""
JOB_STATUS=""
while true; do
  JOB_JSON="$(curl -sS "${AUTH_HEADER[@]}" "${API_V1}/ingest/jobs/${JOB_ID}")"
...
echo "[STEP] 5/6 retrieval with document_id filter"
...
echo "[STEP] 6/6 chat with document_id filter"
