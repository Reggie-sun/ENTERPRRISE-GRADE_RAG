...
class ChatService:  # 封装问答接口的业务逻辑。
    ...
    def answer(self, request: ChatRequest, *, auth_context: AuthContext | None = None) -> ChatResponse:  # 根据用户问题生成问答响应。
        citations = self._build_citations(request, auth_context=auth_context)  # 先统一构造引用列表。
    ...
    def _build_citations(self, request: ChatRequest, *, auth_context: AuthContext | None = None) -> list[Citation]:  # 统一执行检索+重排并产出引用列表。
