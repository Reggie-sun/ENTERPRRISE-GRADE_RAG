...
class DocumentService:  # 封装文档上传、列表和入库的业务逻辑。
    ...
    async def create_document(  # 创建文档记录和 ingest job，并立即返回 queued。
        ...
        *,
        ...
        if duplicate_record is not None:
            latest_job_status = self._get_latest_job_status(duplicate_record.latest_job_id)  # 读取最近一次任务状态，判断是否可自动补发。
            ...
            if latest_job_status in IN_FLIGHT_JOB_STATUSES and duplicate_record.latest_job_id:
                # 同文档任务已经在执行时复用当前 job，避免一次重复点击产生多条并发入库任务。
    ...
    def get_document_preview_file(self, doc_id: str, *, auth_context: AuthContext | None = None) -> tuple[Path, str, str]:  # 返回文档在线预览文件流所需信息。
    ...
    def get_document_file(self, doc_id: str, *, auth_context: AuthContext | None = None) -> tuple[Path, str, str]:  # 返回原始文件下载流所需信息。
        record = self._load_document_record(doc_id)  # 读取文档元数据。
        ...
        media_type = PREVIEW_CONTENT_TYPE_BY_SUFFIX.get(suffix, "application/octet-stream")
        return source_path, media_type, record.file_name
    ...
    def delete_document(self, doc_id: str, *, auth_context: AuthContext | None = None) -> DocumentDeleteResponse:  # 删除文档：先更新主数据状态，再清理向量副本。
        ...
        return DocumentDeleteResponse(doc_id=record.doc_id, status="deleted", vector_points_removed=removed)
    ...
    def rebuild_document_vectors(self, doc_id: str, *, auth_context: AuthContext | None = None) -> DocumentRebuildResponse:  # 重建文档向量：先清理旧向量，再复用 ingest job 机制重建。
        ...
        if record.latest_job_id:  # 已有任务时，先检查是否存在进行中的任务，避免重复并发重建。
            try:
                ...
                if latest_job.status in ({"queued"} | PROCESSING_JOB_STATUSES):
                    raise HTTPException(
        ...
        return DocumentRebuildResponse(  # 返回重建任务信息，前端可据此轮询状态。
            doc_id=record.doc_id,
    ...
    def _enforce_document_create_scope(
        ...
        *,
        ...
        if auth_context.user.role_id == "employee":
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Current role cannot create or manage documents.")
    ...
    def _get_latest_job_status(self, latest_job_id: str | None) -> str | None:  # 读取最近一次 job 状态，缺失或异常时返回 None。
        if not latest_job_id:
            return None
        try:
            return self._load_job_record(latest_job_id).status
