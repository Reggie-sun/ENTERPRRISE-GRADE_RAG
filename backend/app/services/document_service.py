...
from ..rag.parsers.document_parser import SUPPORTED_PARSE_SUFFIXES  # 复用解析层定义的整体支持后缀，避免上传白名单和实际解析能力漂移。
...
PARSED_TEXT_PREVIEW_SUFFIXES = {".docx", ".csv", ".html", ".json", ".xlsx", ".pptx", ".xls"}  # 需要先解析成纯文本再做预览的扩展名集合。
...
TERMINAL_JOB_STATUSES = {"completed", "dead_letter", "partial_failed"}  # ingest job 终态集合，partial_failed 代表有可用结果但存在 OCR 告警。
...
class DocumentService:  # 封装文档上传、列表和入库的业务逻辑。
        ...
        *,
        ...
        if duplicate_record is not None:
            ...
            if latest_job_record is not None and latest_job_record.status == "completed":
                self._save_document_record(duplicate_record)  # 已完成的重复文档直接复用历史结果，但仍要落盘最新文件名和源路径。
                self._record_document_event(
                    action="create",
                    outcome="success",
                    auth_context=auth_context,
            ...
            if self._should_reuse_inflight_job(latest_job_record):
                ...
                self._save_document_record(duplicate_record)  # 复用 in-flight job 时也要持久化本次最新源文件和元数据。
                self._record_document_event(
                    action="create",
                    outcome="success",
                    auth_context=auth_context,
    ...
    def run_ingest_job(self, job_id: str) -> IngestJobStatusResponse:  # 执行指定 ingest job，并更新文档与任务状态。
        ...
        try:  # 尝试执行完整入库流程。
            with self.asset_store.materialize_for_processing(
                ...
                ingestion_result = self.ingestion_service.ingest_document(  # 调用入库服务执行解析、切块、向量化和写库。
        ...
        if ingestion_result.final_status == "partial_failed":  # OCR fallback 有非致命告警时，保留可用结果并显式标记 partial_failed。
            self._update_job_record(
                job_record,
    ...
    async def save_upload(self, upload: UploadFile) -> DocumentUploadResponse:  # 保存上传文件并执行完整入库。
        ...
        return DocumentUploadResponse(  # 把入库结果组装成接口响应。
            ...
            status="partial_failed" if ingestion_result.final_status == "partial_failed" else "ingested",  # OCR 非致命告警时显式返回 partial_failed。
            ...
            warning_message=ingestion_result.warning_message,  # 把 OCR fallback 告警直接回给同步上传调用方。
    ...
    def list_documents(  # 列出当前已经上传过的文档，并支持分页与筛选。
        ...
        *,
        ...
        records = self._list_document_records()  # 统一加载文档记录，兼容本地 JSON 与 PostgreSQL 元数据。
    ...
    def _has_materialized_ingest_artifacts(self, doc_id: str) -> bool:  # 判断当前 doc_id 是否已经产出可复用的解析/切块结果，避免被悬空 queued job 锁住。
