...
TEXT_PREVIEW_SUFFIXES = {".txt", ".md", ".markdown"}  # 支持直接文本预览的扩展名集合。
PDF_PREVIEW_SUFFIXES = {".pdf"}  # 支持在线 PDF 预览的扩展名集合。
SUPPORTED_PREVIEW_SUFFIXES = TEXT_PREVIEW_SUFFIXES | PDF_PREVIEW_SUFFIXES  # 统一预览支持集合。
TEXT_PREVIEW_MAX_CHARS = 12_000  # 文本预览最大字符数，避免一次返回过大响应。
PREVIEW_CONTENT_TYPE_BY_SUFFIX = {  # 预览接口的内容类型映射。
...
class DocumentService:  # 封装文档上传、列表和入库的业务逻辑。
    ...
    async def create_document(  # 创建文档记录和 ingest job，并立即返回 queued。
        ...
        *,
        ...
        if duplicate_record is not None:
            if self._should_reingest_duplicate_document(duplicate_record):  # 老文档元数据还在，但当前 collection 已无向量时自动补一次新 job。
                self._refresh_duplicate_document_source(duplicate_record, content=content)  # 用这次重新上传的内容刷新源文件，避免旧 storage_path 已失效时补发任务仍读不到文件。
                followup_job = self._create_followup_ingest_job(duplicate_record)  # 沿用已有 doc_id，只补发新的 ingest job。
    ...
    def run_ingest_job(self, job_id: str) -> IngestJobStatusResponse:  # 执行指定 ingest job，并更新文档与任务状态。
        ...
        resolved_source_path = self._resolve_storage_path(document_record.storage_path)  # 兼容容器/宿主机切换后旧绝对路径失效的场景。
        if str(resolved_source_path) != document_record.storage_path:  # 命中了兼容路径时，把元数据也一起修正到当前可访问路径。
            document_record.storage_path = str(resolved_source_path)
        ...
        try:  # 尝试执行完整入库流程。
            self.ingestion_service.ingest_document(  # 调用入库服务执行解析、切块、向量化和写库。
                ...
                source_path=resolved_source_path,  # 传入当前运行环境可访问的原始文件路径。
    ...
    def get_document_preview(self, doc_id: str, *, max_chars: int = TEXT_PREVIEW_MAX_CHARS) -> DocumentPreviewResponse:  # 按 doc_id 读取在线预览内容。
        ...
        if suffix in TEXT_PREVIEW_SUFFIXES:  # 文本类文档直接读取原文件内容用于在线预览。
            ...
            text_content, text_truncated = self._read_text_preview(source_path, max_chars=max_chars)
            return DocumentPreviewResponse(
                doc_id=record.doc_id,
                file_name=record.file_name,
                preview_type="text",
                content_type=PREVIEW_CONTENT_TYPE_BY_SUFFIX[suffix],
        ...
        parsed_text, text_truncated = self._read_text_preview(parsed_path, max_chars=max_chars) if parsed_path.exists() else ("", False)
        return DocumentPreviewResponse(
            doc_id=record.doc_id,
            file_name=record.file_name,
            preview_type="pdf",
    ...
    def get_document_preview_file(self, doc_id: str) -> tuple[Path, str, str]:  # 返回文档在线预览文件流所需信息。
        ...
        source_path = self._resolve_storage_path(record.storage_path)  # 兼容容器/宿主机路径切换后的历史路径。
        if not source_path.exists():
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Preview source file not found for document: {doc_id}",
            )
        return source_path, PREVIEW_CONTENT_TYPE_BY_SUFFIX[suffix], record.file_name
    ...
    def _get_latest_ingest_status(self, latest_job_id: str | None) -> str | None:  # 读取最近一次 ingest 状态，缺失或异常时返回 None。
        if not latest_job_id:
            return None
        try:
            return self._load_job_record(latest_job_id).status
    ...
    def list_documents(  # 列出当前已经上传过的文档，并支持分页与筛选。
        ...
        *,
        ...
        for record in records:  # 把 document 记录转换成对外的摘要结构。
            latest_ingest_status = self._get_latest_ingest_status(record.latest_job_id)  # 最近一次 ingest 状态单独返回，避免和文档状态混用。
            items.append(
                DocumentSummary(
    ...
    def _should_reingest_duplicate_document(self, record: DocumentRecord) -> bool:  # 判断重复文档是否需要补发一次新 ingest job。
        latest_job_id = record.latest_job_id.strip()  # 拿到当前文档最近一次 job ID。
        if not latest_job_id:  # 缺少最新 job 时不走自动补发，保持原有 409 语义。
            return False
        try:
            latest_job = self._load_job_record(latest_job_id)  # 读取最近一次入库任务状态。
        except HTTPException:  # 元数据不完整时保守返回 False，避免误补发。
            return False
        if latest_job.status != "completed":  # 只对“已完成但向量缺失”的文档补发新任务。
            return False
        return not self.ingestion_service.vector_store.has_document_points(record.doc_id)  # 当前 collection 无向量时才重建 job。
    ...
    def _refresh_duplicate_document_source(self, document_record: DocumentRecord, *, content: bytes) -> None:  # 用本次重新上传的内容刷新重复文档的源文件，确保补发任务总能读取到当前文件。
        ...
        target_path = self.settings.upload_dir / target_name  # 把源文件统一写回当前运行环境的 upload_dir。
        target_path.parent.mkdir(parents=True, exist_ok=True)  # 确保上传目录存在。
        target_path.write_bytes(content)  # 直接用这次上传的真实内容覆盖/补齐源文件。
        document_record.storage_path = str(target_path)  # 后续 worker 统一读取当前可访问的新路径。
        document_record.updated_at = datetime.now(timezone.utc)  # 刷新更新时间，反映这次源文件修复。

    def _resolve_storage_path(self, storage_path: str) -> Path:  # 把历史元数据里的原始文件路径映射到当前运行环境可访问的位置。
        stored_path = Path(storage_path)  # 先按原值构造成 Path。
        if stored_path.exists():  # 旧路径在当前环境可直接访问时，不做任何改动。
            return stored_path
        fallback_path = self.settings.upload_dir / stored_path.name  # 容器模式下优先按当前 upload_dir + 原文件名回退。
        if fallback_path.exists():  # 命中同名文件时，说明只是宿主机绝对路径失效，可以安全重写。
            return fallback_path
        return stored_path  # 仍不存在时返回原路径，让后续错误保留真实上下文。
    ...
    @staticmethod
    def _read_text_preview(path: Path, *, max_chars: int) -> tuple[str, bool]:  # 读取文本预览并按最大长度截断。
        raw_text = path.read_text(encoding="utf-8", errors="ignore")  # 用 ignore 兼容脏字符，避免单文件编码问题打断页面。
