...
class DocumentService:  # 封装文档上传、列表和入库的业务逻辑。
    ...
    def delete_document(self, doc_id: str) -> DocumentDeleteResponse:  # 删除文档：先更新主数据状态，再清理向量副本。
    ...
    def rebuild_document_vectors(self, doc_id: str) -> DocumentRebuildResponse:  # 重建文档向量：先清理旧向量，再复用 ingest job 机制重建。
        record = self._load_document_record(doc_id)  # 先读取文档元数据。
