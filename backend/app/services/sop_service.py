...
class SopService:  # v0.4 SOP 主数据服务：先承接独立对象、权限过滤和字段归一，不侵入文档入库链路。
    ...
    def list_sops(
        ...
        *,
        ...
        if (
            ...
            and self.settings.department_query_isolation_enabled
    ...
    @staticmethod
    def _downloadable_formats(record: SopRecord) -> list[SopDownloadFormat]:
        formats: list[SopDownloadFormat] = []
...
"""
    ...
    def _build_pdf_bytes(renderable: SopRenderableDocument) -> bytes:
        ...
        except ImportError as exc:  # pragma: no cover - 依赖缺失时直接给出明确错误，避免导出成损坏 PDF。
        ...
        pdf.setTitle(renderable.title)
        pdf.setAuthor("Enterprise RAG")
        pdf.setSubject("SOP Draft Export")
        pdf.setCreator("Enterprise RAG")

        y = page_height - margin_y
        for raw_line in SopService._compose_download_lines(renderable):
            wrapped_lines = SopService._wrap_pdf_line(
                raw_line,
                font_name=font_name,
                font_size=font_size,
                max_width=max_width,
                pdfmetrics_module=pdfmetrics,
            )
    ...
    def _wrap_pdf_line(
        ...
        *,
        ...
        for char in text:
            ...
            if current and pdfmetrics_module.stringWidth(candidate, font_name, font_size) > max_width:
