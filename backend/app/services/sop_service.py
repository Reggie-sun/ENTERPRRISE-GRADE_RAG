...
class SopService:  # v0.4 SOP 主数据服务：先承接独立对象、权限过滤和字段归一，不侵入文档入库链路。
    ...
    def get_sop_preview_file(self, sop_id: str, *, auth_context: AuthContext | None = None) -> SopFilePayload:
        ...
        if record.preview_resource_path.startswith("preview://"):
            content = self._build_html_preview_bytes(record) if record.preview_resource_type == "html" else self._build_pdf_bytes(record)
        ...
        *,
        ...
        if resource_path.startswith("asset://"):
            if download_format == "docx":
                content = self._build_docx_bytes(record)
            else:
                content = self._build_pdf_bytes(record)
            return SopFilePayload(filename=filename, media_type=media_type, content=content)
    ...
    @staticmethod
    def _build_docx_bytes(record: SopRecord) -> bytes:
        text_lines = SopService._compose_download_lines(record)
...
<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:dcmitype="http://purl.org/dc/dcmitype/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  <dc:title>{html.escape(record.title)}</dc:title>
  <dc:creator>{html.escape(record.updated_by or record.created_by or "Enterprise RAG")}</dc:creator>
</cp:coreProperties>
"""
...
"""
    ...
    @staticmethod
    def _build_pdf_bytes(record: SopRecord) -> bytes:
        lines = [line.encode("ascii", errors="replace").decode("ascii") for line in SopService._compose_download_lines(record)]
    ...
    @staticmethod
    def _build_html_preview_bytes(record: SopRecord) -> bytes:
        body_lines = []
        for line in SopService._compose_download_lines(record):
...
"""
    ...
    @staticmethod
    def _compose_download_lines(record: SopRecord) -> list[str]:
        lines = [
            record.title,
            f"SOP ID: {record.sop_id}",
            f"Department: {record.department_id}",
            f"Process: {record.process_name or '-'}",
            f"Scenario: {record.scenario_name or '-'}",
            f"Version: v{record.version}",
            "",
        ]
        preview_lines = (record.preview_text_content or "No preview text available.").splitlines()
