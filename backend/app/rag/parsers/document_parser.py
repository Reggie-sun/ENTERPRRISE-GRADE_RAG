...
from pypdf.errors import EmptyFileError, PdfReadError, PdfStreamError  # 把底层 PDF 解析异常转成业务可处理的错误。

SUPPORTED_NATIVE_PARSE_SUFFIXES = {".pdf", ".md", ".markdown", ".txt", ".doc", ".docx", ".csv", ".html", ".json", ".xlsx", ".pptx", ".xls"}  # 当前系统支持原生解析的文件扩展名集合。
...
LIBREOFFICE_BINARY_ENV_VARS = ("RAG_LIBREOFFICE_BINARY", "LIBREOFFICE_BINARY")
LIBREOFFICE_BINARY_COMMANDS = ("soffice", "libreoffice", "lowriter")
...
class DocumentParser:  # 负责把不同格式的文件统一解析成纯文本。
    def parse(  # 根据文件类型选择对应解析逻辑。
        ...
        *,
        ...
        if suffix == ".pdf":  # 如果是 PDF 文件。
            ...
            text = self._parse_doc(source_path)
            parser_name = "doc_libreoffice"
    ...
    def extract_pdf_page_texts(source_path: Path) -> list[str]:  # 单独暴露 PDF 分页原生文本，给按页 OCR fallback 复用。
        ...
        except (EmptyFileError, PdfReadError, PdfStreamError) as exc:
            raise ValueError(f"Invalid PDF content for '{source_path.name}'.") from exc
    ...
    @staticmethod
    def _parse_doc(source_path: Path) -> str:  # 先把旧版 .doc 转成 .docx，再复用现有 DOCX 解析链路。
        soffice_path = find_libreoffice_binary()
        if soffice_path is None:
            raise RuntimeError(
                "Legacy .doc parsing requires a resolvable LibreOffice binary in the current ingest runtime."
            )

        with TemporaryDirectory(prefix="doc_convert_") as temp_dir:
            try:
                ...
                raise RuntimeError(f"LibreOffice conversion timed out for '{source_path.name}'.") from exc
            converted_path = Path(temp_dir) / f"{source_path.stem}.docx"
            if completed.returncode != 0 or not converted_path.exists():
                error_output = (completed.stderr or completed.stdout).strip() or "unknown conversion error"
                raise ValueError(f"Failed to convert legacy DOC '{source_path.name}' to DOCX: {error_output}")
            return DocumentParser._parse_docx(converted_path)
...
def get_configured_libreoffice_binary() -> str | None:
    try:
        from ...core.config import get_settings
    except Exception:
        return None

    configured = (get_settings().libreoffice_binary or "").strip()
...
def _resolve_executable_candidate(candidate: str | None) -> str | None:
    ...
    if not normalized:
        return None
    resolved = shutil.which(normalized) if "/" not in normalized else normalized
...
def find_libreoffice_binary() -> str | None:
    seen: set[str] = set()
    candidates = (
        *(os.getenv(name) for name in LIBREOFFICE_BINARY_ENV_VARS),
        get_configured_libreoffice_binary(),
        *(shutil.which(command) for command in LIBREOFFICE_BINARY_COMMANDS),
        *LIBREOFFICE_BINARY_CANDIDATES,
    )
