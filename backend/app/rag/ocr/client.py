...
from dataclasses import dataclass  # 导入 dataclass，用于封装 OCR 结果。
...
class OCRClient:  # 封装 OCR provider 的最小统一入口。
    ...
    def extract_image_text(self, *, source_path: Path, filename: str) -> OCRExtractionResult:  # 对图片执行 OCR。
        ...
        if provider == "paddleocr":
            return OCRExtractionResult(parser_name="image_ocr_paddle", text=self._extract_image_text_with_paddle(source_path))
    ...
    def _extract_with_mock(self, *, source_path: Path, filename: str, parser_name: str) -> OCRExtractionResult:  # 测试和联调阶段用 sidecar 文本模拟 OCR 输出。
        for candidate in (
            ...
            if candidate.exists():
                return OCRExtractionResult(parser_name=parser_name, text=candidate.read_text(encoding="utf-8"))
        return OCRExtractionResult(parser_name=parser_name, text=f"Mock OCR text extracted from {filename}")

    def _extract_image_text_with_paddle(self, source_path: Path) -> str:  # 让 PaddleOCR 直接识别图片。
        ocr = self._get_paddle_ocr()
        results = ocr.ocr(str(source_path), cls=self.settings.ocr_paddle_use_angle_cls)
        return self._flatten_paddle_results(results)
            ...
            with fitz.open(str(source_path)) as pdf:
                for page_index, page in enumerate(pdf, start=1):
                    ...
                    try:
                        ...
                        page_text = self._extract_image_text_with_paddle(image_path).strip()
                        if page_text:
    ...
    @staticmethod
    def _flatten_paddle_results(results: Any) -> str:  # 把 PaddleOCR 嵌套输出拍平成统一文本。
        lines: list[str] = []
