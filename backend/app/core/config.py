...
from ._auth import DEFAULT_AUTH_TOKEN_SECRET, _AuthSettings
...
class Settings(_AppSettings, _StorageSettings, _ModelSettings, _RetrievalSettings, _OCRSettings, _AuthSettings):
    ...
    @model_validator(mode="after")
    def validate_production_security_defaults(self) -> "Settings":
        """在 staging/production 环境强制启用更安全的默认配置。"""
        normalized_env = self.app_env.strip().lower()
        if normalized_env not in {"staging", "production"}:
            return self

        if self.auth_token_secret == DEFAULT_AUTH_TOKEN_SECRET:
            raise ValueError("RAG_AUTH_TOKEN_SECRET must be explicitly configured outside development/test.")
        if self.debug:
        ...
        if self.auth_identity_bootstrap_public_enabled:
            raise ValueError("auth_identity_bootstrap_public_enabled must be false outside development/test.")

        runtime_store_url = (self.auth_runtime_store_url or self.celery_broker_url).strip()
        if not runtime_store_url:
            raise ValueError("A shared auth runtime store is required outside development/test.")
        if urlparse(runtime_store_url).scheme not in {"redis", "rediss"}:
            raise ValueError("A Redis-backed auth runtime store is required outside development/test.")
        return self
