...
from ..schemas.request_trace import RequestTraceRecord


class RequestTraceRepository(ABC):
    @abstractmethod
    def append(self, record: RequestTraceRecord) -> None:
...
class FilesystemRequestTraceRepository(RequestTraceRepository):
    def __init__(self, request_trace_dir: Path) -> None:
        self.request_trace_dir = request_trace_dir
        self.request_trace_dir.mkdir(parents=True, exist_ok=True)
        self._lock = threading.Lock()

    def append(self, record: RequestTraceRecord) -> None:
        target_path = self.request_trace_dir / f"{record.occurred_at.date().isoformat()}.jsonl"
    ...
    def list_records(
        ...
        *,
        ...
        for path in sorted(self.request_trace_dir.glob("*.jsonl"), reverse=True):
            with path.open("r", encoding="utf-8") as handle:
                for line in handle:
                    normalized = line.strip()
                    if not normalized:
                        continue
                    record = RequestTraceRecord.model_validate_json(normalized)
