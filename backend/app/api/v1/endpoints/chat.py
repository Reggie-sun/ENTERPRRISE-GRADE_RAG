...
def ask_question(  # 定义问答接口函数。
    ...
    auth_context: AuthContext | None = Depends(get_optional_auth_context),  # 已登录时问答引用按部门范围过滤。
...
def ask_question_stream(  # 定义流式问答接口函数。
    request: ChatRequest,  # 从请求体中接收用户问题和 top_k 参数。
    auth_context: AuthContext | None = Depends(get_optional_auth_context),  # 已登录时流式问答与普通问答复用同一份权限上下文。
    chat_service: ChatService = Depends(get_chat_service),  # 通过依赖注入获取问答服务实例。
) -> StreamingResponse:
