...
def _sync_document_identifier_payload(data: object) -> object:
    if not isinstance(data, dict):
        return data
    document_id = data.get("document_id")
    doc_id = data.get("doc_id")
    effective = document_id or doc_id
    if effective is not None:
        data.setdefault("document_id", effective)
        data.setdefault("doc_id", effective)
    return data


class DocumentCreateResponse(BaseModel):  # 定义 POST /documents 的响应结构。
    document_id: str  # 新创建文档的唯一标识。
    doc_id: str  # 兼容历史前端字段，后续逐步收敛到 document_id。
    job_id: str  # 本次入库任务的唯一标识。
    status: Literal["queued", "completed"] = "queued"  # 新文档默认 queued；已完成的重复文档可直接复用 completed。

    @model_validator(mode="before")
    @classmethod
    def sync_document_identifier(cls, data: object) -> object:
        return _sync_document_identifier_payload(data)


class DocumentBatchItemResponse(BaseModel):  # 定义批量上传场景里单个文件的创建结果。
    file_name: str  # 当前文件名。
    status: Literal["queued", "failed"]  # 当前文件处理结果。
    document_id: str | None = None  # 成功创建时返回文档 ID（当前主字段）。
    ...
    def validate_item_shape(self) -> "DocumentBatchItemResponse":  # 约束 queued/failed 两类结果字段语义。
        if self.status == "queued":
            if self.document_id is None or self.doc_id is None or self.job_id is None:
                raise ValueError("queued item requires document_id/doc_id and job_id")
...
class DocumentRecord(BaseModel):  # 定义 document 元数据持久化结构。
    ...
    retrieval_department_ids: list[str] = Field(default_factory=list)  # 检索补充授权：允许哪些部门在检索场景下将本文档作为跨部门补充来源。
...
class DocumentDeleteResponse(BaseModel):  # 定义 DELETE /documents/{doc_id} 的响应结构。
    document_id: str  # 被删除的文档 ID（当前主字段）。
    doc_id: str  # 兼容历史前端字段。
...
class DocumentBatchDeleteResponse(BaseModel):  # 定义 DELETE /documents 的批量删除响应结构。
    deleted_count: int = Field(ge=0)  # 成功删除的文档数量。
    failed_count: int = Field(ge=0)  # 删除失败的文档数量。
