...
def test_create_document_rejects_duplicate_file_hash_within_same_tenant(tmp_path: Path) -> None:  # 测试同租户重复上传相同内容时会返回 409，避免重复入库。
    settings = build_test_settings(tmp_path)  # 构造测试配置。
    ensure_data_directories(settings)  # 创建测试目录。
    service = DocumentService(settings)  # 创建测试服务实例。
    ...
    assert second_response.status_code == 409  # 第二次相同内容上传应返回冲突。
    detail = second_response.json()["detail"]  # 提取错误详情。
    assert detail["code"] == "DOCUMENT_ALREADY_EXISTS"  # 错误码应明确标识为重复文档。
    assert detail["doc_id"] == first_response.json()["doc_id"]  # 冲突响应应指向首次创建的文档。
    assert len(list(settings.document_dir.glob("*.json"))) == 1  # documents 元数据目录里不应多出重复文档。
    assert len(list(settings.job_dir.glob("*.json"))) == 1  # jobs 元数据目录里也不应多出重复任务。
...
def test_create_document_requeues_duplicate_with_fresh_upload_when_legacy_source_is_missing(tmp_path: Path) -> None:  # 老文档已完成但源文件丢失时，重新上传相同内容应刷新源文件并允许新 job 完成。
    ...
    assert refreshed_source_path == settings.upload_dir / f"{first_payload['doc_id']}__manual.txt"  # 重复补发应把源文件重建到当前 upload_dir，而不是继续引用坏掉的旧绝对路径。
...
def test_duplicate_file_hash_detection_uses_metadata_store(tmp_path: Path) -> None:  # 验证重复内容查重在 metadata store 模式下依旧生效。
    ...
    assert first_response.status_code == 201
    assert second_response.status_code == 409  # 第二次应命中重复内容冲突。
