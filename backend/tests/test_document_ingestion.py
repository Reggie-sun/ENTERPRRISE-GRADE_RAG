...
def test_run_ingest_job_updates_status_and_document_state(tmp_path: Path) -> None:  # 测试手动触发 ingest job 后会完成入库并更新文档状态。
    settings = build_test_settings(tmp_path)  # 构造测试配置。
    ensure_data_directories(settings)  # 创建测试目录。
    service = DocumentService(settings)  # 创建测试服务实例。
    ...
    assert run_payload["status"] == "completed"  # 任务应执行完成。
    ...
    assert service.ingestion_service.vector_store.count_points() >= 1  # 任务执行后应至少写入一条向量。
