...
企业内部已经沉淀了一批标准 SOP 文档，但在实际使用中仍存在这些问题：

- SOP 文档分散在不同目录或系统中
    
- 不同部门、工序、业务场景下缺少统一分类入口
    
- 一线人员难以快速定位到当前场景对应的标准 SOP
    
- 已生成的 SOP 文档缺少统一在线预览与导出能力
    

### 业务需求

需要在系统中增加 **SOP 查看与下载** 能力，实现：

1. 按部门、工序/场景分类展示已生成的标准 SOP 文档，便于用户快速查找
    
2. 支持在线预览 SOP 内容，减少反复下载和本地打开的操作成本
    
3. 支持一键下载为 Word 或 PDF 格式，方便培训、归档和线下执行
...
- 当前《需求规格说明书（V1.0 精简版）》对应的交付边界以 **BS 架构、内网私有化、无外网依赖** 为准
    
- 上述混合云部署形态继续保留为后续扩展方案，不作为当前 V1 必须交付前提
...
## V1 业务边界补充（依据需求规格说明书 V1.0）

### 1. 当前 V1 交付形态

- 浏览器 BS 架构，优先保证 PC 端可直接访问
    
- 内网私有化部署，无外网 API 依赖
    
- 企业微信机器人作为补充入口，不替代浏览器主入口
    

### 2. 核心用户与角色边界

- 普通员工：知识问答、文档检索、SOP 查看与下载、个人历史与收藏
    
- 部门管理员：维护本部门文档、生成与管理本部门 SOP
    
- 系统管理员：系统配置、用户权限、全局日志与审计管理
...
- 员工端：智能问答、文档检索、SOP 查看下载、个人中心
    
- 管理后台：文档管理、部门与分类管理、SOP 生成管理、权限管理、日志查看、系统配置
    

### 4. 后端能力补充

在现有 RAG 主链路之外，V1 还需要在业务架构上明确这些服务边界：

- 用户认证与接口鉴权
    
- 文档解析、分块、异步向量构建与元数据关联
    
- RAG 检索、重排、回答生成、来源追溯与兜底回复
    
- SOP 智能生成、在线编辑、导出、版本管理与来源引用
...
- 权限隔离、日志可追溯、密码安全存储属于 V1 必须考虑的基础能力
...
这里要先统一一个原则：  
允许高并发，不等于把并发无限放开。  
真正稳的做法不是“把 GPU 吃满”，而是：

`准入控制 + 受控批处理 + 在线优先 + 分级降级 + 回压限流`

**V1 推荐的基本策略**

- GPU0 只负责在线 LLM 生成，不接 OCR、embedding、离线评测
    
- GPU1 优先服务在线 query embedding 和 rerank，离线 embedding 只能吃剩余预算
    
- 高峰期允许请求排队，但不允许所有请求一起抢占 GPU
    
- 先降参数、再降能力、最后才报错，不能一上来直接 OOM
    

**在线请求的准入控制建议**

每个问答请求进入生成层前，都先做一次成本估算：

`请求进入 -> 估算 prompt tokens / context chunks / max_new_tokens -> 判断 GPU 预算 -> 允许进入 / 排队 / 参数降级 / fallback`
...
如果预算不足，不要继续硬塞进 vLLM 队列，而要优先做下面这些动作：

1. 关闭或减弱 multi-query / rewrite
    
2. 减少召回候选和 rerank 候选数
    
3. 减少最终送入 LLM 的 chunk 数
    
4. 下调 `max_new_tokens`
    
5. fallback 次选模型或返回 extractive answer
    

**受控批处理，而不是无限并发**

高并发下要靠 continuous batching 或框架自己的批处理能力提升吞吐，但前提是：

- 批处理要受最大并发限制保护
    
- 不能为了吞吐把长上下文和短上下文无限混跑
    
- 不能让单个超长请求长期阻塞一批普通请求
    

也就是说，系统应该追求“稳定吞吐”，不是追求“瞬时把卡打满”。
...
2. 在线 rerank / query embedding
    
3. 报警推送、企业微信结果回传这类时效任务
...
只要在线请求排队时间持续升高，就应该先让离线任务降速，而不是继续挤占 GPU。

**高负载时的动态降级顺序**

建议固定成一条链路，不要每次临时拍脑袋：

`高负载 -> 降 query 扩展 -> 降 rerank 成本 -> 降 context -> 降生成长度 -> fallback 小模型 -> extractive fallback`
...
- 单请求平均 context tokens
    
- 参数降级次数
    
- budget reject / budget queue 次数
    
- 各阶段 OOM 次数
    

如果这些指标没单独看，你只看 GPU 使用率，通常看不出真正的瓶颈。

**一句话版本**

高并发下真正稳的 GPU 方案不是“放开并发”，而是：

`严格分卡 -> 在线优先 -> 准入控制 -> 受控批处理 -> 参数降级 -> fallback -> 回压限流`
...
企业 RAG 真正跑起来后，GPU 压力不只来自问答，还来自：

- 在线问答生成
    
- 在线 query embedding / rerank
...
2. 在线 query embedding / rerank
    
3. 报警消息推送、企业微信结果回传
    
4. 离线文档 embedding / reingest
...
- OCR、embedding、rerank、LLM 在同一张 GPU 上互相抢卡
...
不要把 GPU 预算设计成“谁先来谁用”，而是：

- 先为在线问答预留固定预算
    
- 离线任务只使用剩余预算
    
- 当 GPU 超过阈值时，离线任务自动减 batch 或暂停
    
- 当在线请求排队时间超过阈值时，优先触发离线限流
    

如果在线链路已经开始排队，系统的第一反应应该是“压离线”，不是“继续加离线吞吐”。

**一句话版本**

调度层的原则必须写死为：

`在线优先、离线让路、超阈值自动限流、预算不足自动降级`
